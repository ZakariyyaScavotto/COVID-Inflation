...
def generateNonAvgPlots(df, filename):
    ...
    axs[1].legend()
    axs[2].legend()
    fig.savefig('Metrics/PlotsNoLR/'+filename.replace('.xlsx', '.png').replace('Metrics/',''))
...
def generateAvgPlots(df, filename):
    ...
    # save the figure
    fig.savefig('Metrics/PlotsNoLR/'+filename.replace('.xlsx', 'Avg.png').replace('Metrics/',''))
...
def mainPlotting():
    ...
    print('Plotting Program (No LR) Done')
