...
def trainEvalNN(window, testWindow, loadModel=False):
    if not loadModel:
        ...
        model = nnWithReversible(numDenseLayers=analysis.best_config["numDenseLayers"], dropout=analysis.best_config["dropout"])
        train_loader, test_loader = get_data_loaders(window, testWindow, analysis.best_config["batch_size"], loadModel)
        optimizer = optim.SGD(model.parameters(), lr=analysis.best_config["lr"])
        errors = []
...
def main():
    ...
    train.to_excel("Metrics/newNNTrain.xlsx")
    test.to_excel("Metrics/newNNTest.xlsx")
    print("Program Done")
