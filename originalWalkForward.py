...
def compileMetrics(metricsDict, loadModel):
...
def newTrainEvalNN(window, testWindow, loadModel=False):
    if os.path.exists("nnProject"):
        shutil.rmtree("nnProject")
        print("Old NN project deleted")
    xTrain, yTrain, xTest, yTest = makeTrainTest("NN", window, testWindow)
    if loadModel:
...
def trainEvalRNN(window, testWindow, loadModel=False):
    ...
    xTrain, yTrain, xTest, yTest = makeTrainTest("RNN", window, testWindow)
...
def main():
    ...
    # compile all the train metrics and test metrics into one dataframe each
    trainMetrics = pd.concat(trainMetrics)
    testMetrics = pd.concat(testMetrics)
    # add a row at the end which is the average of each column for rows containing "LR"
    trainMetrics.loc['LRavg'] = trainMetrics.loc[trainMetrics.index.str.contains("LR")].mean()
    testMetrics.loc['LRavg'] = testMetrics.loc[testMetrics.index.str.contains("LR")].mean()
    # add a row at the end which is the average of each column for rows containing "RF"
    trainMetrics.loc['RFavg'] = trainMetrics.loc[trainMetrics.index.str.contains("RF")].mean()
    testMetrics.loc['RFavg'] = testMetrics.loc[testMetrics.index.str.contains("RF")].mean()
