...
def makeTrainTest(modelName, window, testWindow, secondTime=False): # Train test but with breaking up between pre-2020 and 2020->beyond
    # Read econ data
    econData = readEconData("Data/ConstructedDataframes/AllEconBrentPlus3COVID.xlsx")
...
def trainEvalLR(window, testWindow, loadModel=False):
    ...
    if loadModel:
        myLR = pickle.load(open("Brent3COVIDModels/LRModel.pickle", "rb"))
        ...
        pickle.dump(myLR, open("Brent3COVIDModels/LRModel.pickle", "wb"))
...
def trainEvalRF(window, testWindow, loadModel=False):
    ...
    sampleWeights = getSampleWeights(window, testWindow)
    if loadModel:
        myRF = pickle.load(open("Brent3COVIDModels/RFModel.pickle", "rb"))
        ...
        pickle.dump(myRF, open("Brent3COVIDModels/RFModel.pickle", "wb"))
...
def trainEvalNN(window, testWindow, loadModel=False):
    ...
    if loadModel:
        # myNN = keras.models.load_model("Brent3COVIDModels/NNModel.h5", custom_objects={'loss': myMSE(sampleWeights)})
        myNN = keras.models.load_model("Brent3COVIDModels/NNModel.h5", compile=False)
        ...
        myNN.save("Brent3COVIDModels/NNModel.h5")
...
def buildNN(sampleWeights, hp):
    ...
    myNN.add(layers.Dense(units = 24, activation='relu', input_shape=[27]))
...
def trainEvalRNN(window, testWindow, loadModel=False):
    ...
    rnnXTrain.reshape(len(rnnXTrain), timestep, 27)
    rnnXTest.reshape(len(rnnXTest), timestep, 27)
    if not loadModel:
        sampleWeights = tf.constant((pd.Series(getSampleWeights(window, testWindow))), dtype=tf.float32)
    else:
        if window != 346:
    ...
    if loadModel:
        # myRNN = keras.models.load_model("Brent3COVIDModels/RNNModel.h5", custom_objects={'loss': myMSE(sampleWeights)})
        myRNN = keras.models.load_model("Brent3COVIDModels/RNNModel.h5", compile=False)
        ...
        myRNN.save("Brent3COVIDModels/RNNModel.h5")
        print("RNN saved")
        if isinstance(trainMAE, pd.Series):
            trainMAE = trainMAE[0]
    else:
...
def buildRNN(sampleWeights, hp):
    ...
    myRNN.add(layers.SimpleRNN(units = hp.Int('units', 1, 60), activation='relu', input_shape=(12,27), dropout = (hp.Float('dropout', 0.2, 0.7, step=0.05)), recurrent_dropout = (hp.Float('recurDropout' , 0.2, 0.7, step=0.05)),return_sequences=False))
...
def makeModel(myNN):
    ...
    for count, layer in enumerate(myNN.layers):
        if count == 0:
            NN.add(layers.Dense(layer.units, activation=layer.activation, input_shape=[27]))
...
def trainEvalEnsemble(window, testWindow, loadModel=False):
    ...
    if loadModel:
        myLR = pickle.load(open("Brent3COVIDModels/EnsembleModel.pickle", "rb"))
        ...
        pickle.dump(myEnsemble, open("Brent3COVIDModels/EnsembleModel.pickle", "wb"))
...
def main():
    ...
    train.to_excel("Brent3COVIDMetrics/rollingTrainMetrics.xlsx")
    test.to_excel("Brent3COVIDMetrics/rollingTestMetrics.xlsx")
    print("Program Done")
    mainPlotting()
