...
"""
Walk-forward validation for inflation forecasting models.
...
Usage:
    ...
    python walkForwardPrePost2020.py
    
    # Train specific models
    python walkForwardPrePost2020.py --models LR RF ARIMA
    
    # Train only neural networks
    python walkForwardPrePost2020.py --models NN RNN
    
    # Load existing models instead of training from scratch
    python walkForwardPrePost2020.py --load-model
    
    # Combine options
    python walkForwardPrePost2020.py --models LR ARIMA NN --load-model
...
def trainEvalARIMA(window, testWindow, loadModel=False):
    ...
    if loadModel:
        myARIMA = pickle.load(open("Models/ARIMAModel.pickle", "rb"))
    ...
    if not loadModel:
        pickle.dump(myARIMA, open("Models/ARIMAModel.pickle", "wb"))
...
def main():
    ...
    if 'ARIMA' in models_to_train:
        # Repeat the above steps for the ARIMA model
        temp = train.loc[train.index.str.contains("Pre2020ARIMAavg")]*10
        temp2 = train.loc[train.index.str.contains("TransARIMAavg")]*5
        temp3 = train.loc[train.index.str.contains("Post2020ARIMAavg")]*2
        for i in range(len(temp2.columns)):
            temp.iloc[0,i] = temp.iloc[0,i] + temp2.iloc[0,i] + temp3.iloc[0,i]
        temp = temp/17
        # Rename the index of temp to TotalARIMAAvg
        temp.rename(index={temp.index[0]:'TotalARIMAAvg'}, inplace=True)
        train = pd.concat([train, temp])
        # train.rename(index={train.index[-1]:'TotalARIMAAvg'}, inplace=True)
        temp = test.loc[test.index.str.contains("Pre2020ARIMAavg")]*10
        temp2 = test.loc[test.index.str.contains("TransARIMAavg")]*5
        temp3 = test.loc[test.index.str.contains("Post2020ARIMAavg")]*2
        for i in range(len(temp2.columns)):
            temp.iloc[0,i] = temp.iloc[0,i] + temp2.iloc[0,i] + temp3.iloc[0,i]
        temp = temp/17
        # Rename the index of temp to TotalARIMAAvg
        temp.rename(index={temp.index[0]:'TotalARIMAAvg'}, inplace=True)
        test = pd.concat([test, temp])
        # test.rename(index={test.index[-1]:'TotalARIMAAvg'}, inplace=True)
    ...
    train_filename = f"Metrics/rollingTrainMetrics_{filename_suffix}.xlsx"
    test_filename = f"Metrics/rollingTestMetrics_{filename_suffix}.xlsx"
