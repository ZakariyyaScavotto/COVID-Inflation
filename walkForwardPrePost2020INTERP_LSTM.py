...
def trainEvalRNN(window, testWindow, loadModel=False):
    ...
    if loadModel:
        ...
        myRNN = keras.models.load_model("InterpModels/LSTMModel.h5", compile=False)
        ...
        print(myRNN.summary())
        print("Finished displaying testing metrics for loaded LSTM")
        myRNN.save("InterpModels/LSTMModel.h5")
        print("LSTM saved")
...
def buildRNN(sampleWeights, hp):
    ...
    myRNN.add(layers.LSTM(units = hp.Int('units', 1, 60), activation='relu', input_shape=(12,23), dropout = (hp.Float('dropout', 0.2, 0.7, step=0.05)), recurrent_dropout = (hp.Float('recurDropout' , 0.2, 0.7, step=0.05)),return_sequences=False))
...
def main():
    ...
    for count, window in enumerate(pre2020trainWindowSize):
        if count == 0:
            metricsDict = { "LSTM": trainEvalRNN(window, pre2020testWindowSize, loadModel)}
            train, test = compileMetrics(metricsDict)
            pre2020trainMetrics.append(train)
            pre2020testMetrics.append(test)
        else:
            metricsDict = { "LSTM": trainEvalRNN(window, pre2020testWindowSize, loadModel=True) }
            train, test = compileMetrics(metricsDict)
            pre2020trainMetrics.append(train)
            pre2020testMetrics.append(test)
    for window in transTrainWindowSize:
        metricsDict = {"LSTM": trainEvalRNN(window, transTestWindowSize, loadModel=True) }
        train, test = compileMetrics(metricsDict)
        transTrainMetrics.append(train)
        transTestMetrics.append(test)
    for count, window in enumerate(post2020trainWindowSize):
        metricsDict = {"LSTM": trainEvalRNN(window, post2020testWindowSize, loadModel=True)}
        train, test = compileMetrics(metricsDict)
    ...
    temp = train.loc[train.index.str.contains("Pre2020LSTMavg")]*20
    temp2 = train.loc[train.index.str.contains("TransLSTMavg")]*16
    temp3 = train.loc[train.index.str.contains("Post2020LSTMavg")]*16
    for i in range(len(temp2.columns)):
        temp.iloc[0,i] = temp.iloc[0,i] + temp2.iloc[0,i] + temp3.iloc[0,i]
    temp = temp/52
    # Rename the index of temp to TotalLSTMAvg
    temp.rename(index={temp.index[0]:'TotalLSTMAvg'}, inplace=True)
    train = pd.concat([train, temp])
    # train.rename(index={train.index[-1]:'TotalLSTMAvg'}, inplace=True)
    temp = test.loc[test.index.str.contains("Pre2020LSTMavg")]*20
    temp2 = test.loc[test.index.str.contains("TransLSTMavg")]*16
    temp3 = test.loc[test.index.str.contains("Post2020LSTMavg")]*16
    for i in range(len(temp2.columns)):
        temp.iloc[0,i] = temp.iloc[0,i] + temp2.iloc[0,i] + temp3.iloc[0,i]
    temp = temp/52
    # Rename the index of temp to TotalLSTMAvg
    ...
    # test.rename(index={test.index[-1]:'TotalLSTMAvg'}, inplace=True)
    # test.rename(index={test.index[-1]:'TotalEnsembleAvg'}, inplace=True)
    # move the rows that contain "Avg" to the bottom
    train = pd.concat([train[~train.index.str.contains("avg")],train[train.index.str.contains("avg")]])
    test = pd.concat([test[~test.index.str.contains("avg")],test[test.index.str.contains("avg")]])
    # save the dataframes to excel files
    train.to_excel("Metrics/InterpTrainMetricsEVERY4_LSTM.xlsx")
    test.to_excel("Metrics/InterpTestMetricsEVERY4_LSTM.xlsx")
    print("Program Done")
    # mainPlotting()
