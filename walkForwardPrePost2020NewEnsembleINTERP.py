...
def getModelMetrics(x, y, model, modelName, training=True):
    ...
    # Convert x to just the values if it is a DataFrame for DESReg
...
def trainEvalEnsemble(window, testWindow, loadModel=False):
    ...
    if loadModel:
        myEnsemble = myEnsembleModel.load("Models/EnsembleModelCustom.pickle") #pickle.load(open("Models/EnsembleModelNew.pickle", "rb"))
        ...
        # myEnsemble = DESRegression(regressors_list=[myLR,myRF])#VotingRegressor(estimators=[('LR', myLR), ('RF', myRF), ('NN', sciNN)])
        myEnsemble = myEnsembleModel(myLR, myRF, "Models/NNModel.h5", sampleWeights, myMSE)
        ...
        # pickle.dump(myEnsemble, open("Models/EnsembleModelNew.pickle", "wb"))
        myEnsemble.save("Models/EnsembleModelCustom.pickle")
...
def main():
    ...
    for count, window in enumerate(pre2020trainWindowSize):
        if count == 0:
            metricsDict = {"LR"+str(window): trainEvalLR(window,pre2020testWindowSize, loadModel), "RF"+str(window): trainEvalRF(window, pre2020testWindowSize, loadModel), "NN"+str(window): trainEvalNN(window, pre2020testWindowSize, loadModel),  "Ensemble": trainEvalEnsemble(window, pre2020testWindowSize, loadModel) }
            train, test = compileMetrics(metricsDict)
            pre2020trainMetrics.append(train)
            pre2020testMetrics.append(test)
        else:
            metricsDict = {"LR"+str(window): trainEvalLR(window,pre2020testWindowSize, loadModel), "RF"+str(window): trainEvalRF(window, pre2020testWindowSize, loadModel), "NN"+str(window): trainEvalNN(window, pre2020testWindowSize, loadModel=True), "Ensemble": trainEvalEnsemble(window, pre2020testWindowSize, loadModel) }
            train, test = compileMetrics(metricsDict)
            pre2020trainMetrics.append(train)
            pre2020testMetrics.append(test)
    for window in transTrainWindowSize:
        metricsDict = {"LR"+str(window): trainEvalLR(window,transTestWindowSize, loadModel), "RF"+str(window): trainEvalRF(window, transTestWindowSize, loadModel), "NN"+str(window): trainEvalNN(window, transTestWindowSize, loadModel=True),  "Ensemble": trainEvalEnsemble(window, transTestWindowSize, loadModel) }
        train, test = compileMetrics(metricsDict)
        transTrainMetrics.append(train)
        transTestMetrics.append(test)
    for count, window in enumerate(post2020trainWindowSize):
        metricsDict = {"LR"+str(window): trainEvalLR(window,post2020testWindowSize, loadModel), "RF"+str(window): trainEvalRF(window, post2020testWindowSize, loadModel), "NN"+str(window): trainEvalNN(window, post2020testWindowSize, loadModel=True), "Ensemble": trainEvalEnsemble(window, post2020testWindowSize, loadModel) }
        train, test = compileMetrics(metricsDict)
    ...
    train.to_excel("Metrics/InterprollingTrainMetricsNewEnsemble.xlsx")
    test.to_excel("Metrics/InterprollingTestMetricsNewEnsemble.xlsx")
    print("Program Done")
    mainPlotting()
